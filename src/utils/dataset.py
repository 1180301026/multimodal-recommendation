...
class RecDataset(object):
    def __init__(self, config, df=None):
        ...
        time_file = os.path.join(self.dataset_path, config['time_file_name'])
        cols = [self.uid_field, self.iid_field, self.splitting_label]
        self.dft = pd.read_csv(time_file, usecols=cols, sep=self.config['field_separator'])
        self.time_inter=[]
        k=-1
        for i in range(len(self.dft)):
            if self.dft[self.uid_field][i]!= k:
                k=self.dft[self.uid_field][i]
                abc=[]
                abc.append(self.dft[self.iid_field][i])
                self.time_inter.append(abc)
            else:
                print(k)
                self.time_inter[k].append(self.dft[self.iid_field][i])
        if df is not None:
            self.df = df
            return
        # if all files exists
